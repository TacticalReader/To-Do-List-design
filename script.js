...
const updateTotals = () => {
  ...
  // FIX #4: Count only incomplete tasks so the greeting is accurate
  totalTasks.innerHTML = tasks.filter((t) => !t.completed).length;
};
...
const renderCategories = () => {
  ...
  categories.forEach((category) => {
    ...
    // FIX #5: Stop the options ⋮ button from triggering the parent card's click (ghost navigation)
    const toggleBtn = div.querySelector(".toggle-btn");
    toggleBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      // Future: open a context menu here (rename, delete category, etc.)
    });
  });
};
...
// FIX #6: Removed unnecessary e.preventDefault() — this is a button click, not a form submit
const addTask = () => {
...
// Event Listeners
// FIX #3: menuBtn still calls toggleScreen but selectedCategory is always pre-set, so it's safe
